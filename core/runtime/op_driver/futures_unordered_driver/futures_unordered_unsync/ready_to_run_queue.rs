...
pub(super) struct ReadyToRunQueue<Fut> {
  ...
  pub(super) head: Cell<*mut Task<Fut>>,
  pub(super) tail: UnsafeCell<*const Task<Fut>>,
  pub(super) stub: Rc<Task<Fut>>,
}
...
impl<Fut> ReadyToRunQueue<Fut> {
  ...
  pub(super) fn enqueue(&self, task: *const Task<Fut>) {
    unsafe {
      debug_assert!((*task).queued.get());

      // This action does not require any coordination
      (*task).next_ready_to_run.set(ptr::null_mut());

      // Note that these atomic orderings come from 1024cores
      let task = task as *mut _;
      let prev = self.head.replace(task);
      (*prev).next_ready_to_run.set(task);
    }
  }
  ...
  pub(super) unsafe fn dequeue(&self) -> Dequeue<Fut> {
    unsafe {
      ...
      let mut next = (*tail).next_ready_to_run.get();
      ...
      if tail == self.stub() {
        ...
        *self.tail.get() = next;
        tail = next;
        next = (*next).next_ready_to_run.get();
      }
      ...
      if self.head.get() as *const _ != tail {
        return Dequeue::Inconsistent;
      }

      self.enqueue(self.stub());

      next = (*tail).next_ready_to_run.get();
    ...
    unsafe {
      loop {
        match self.dequeue() {
          ...
          Dequeue::Data(ptr) => drop(Rc::from_raw(ptr)),
        }
      }
    }
  }
