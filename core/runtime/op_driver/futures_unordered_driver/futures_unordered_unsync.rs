...
pub struct FuturesUnordered<Fut> {
  ready_to_run_queue: Rc<ReadyToRunQueue<Fut>>,
  head_all: Cell<*mut Task<Fut>>,
...
//
// Each task is wrapped in an `Rc` and thereby atomically reference counted.
...
impl<Fut> FuturesUnordered<Fut> {
  ...
  pub fn new() -> Self {
    let stub = Rc::new(Task {
      future: UnsafeCell::new(None),
      next_all: Cell::new(ptr::null_mut()),
      prev_all: UnsafeCell::new(ptr::null()),
      len_all: UnsafeCell::new(0),
      next_ready_to_run: Cell::new(ptr::null_mut()),
      queued: Cell::new(true),
      ready_to_run_queue: Weak::new(),
      woken: Cell::new(false),
    });
    let stub_ptr = Rc::as_ptr(&stub);
    let ready_to_run_queue = Rc::new(ReadyToRunQueue {
      waker: AtomicWaker::new(),
      head: Cell::new(stub_ptr as *mut _),
      tail: UnsafeCell::new(stub_ptr),
      stub,
    });

    Self {
      head_all: Cell::new(ptr::null_mut()),
      ready_to_run_queue,
  ...
  pub fn push(&self, future: Fut) {
    let task = Rc::new(Task {
      ...
      next_all: Cell::new(self.pending_next_all()),
      prev_all: UnsafeCell::new(ptr::null_mut()),
      len_all: UnsafeCell::new(0),
      next_ready_to_run: Cell::new(ptr::null_mut()),
      queued: Cell::new(true),
      ready_to_run_queue: Rc::downgrade(&self.ready_to_run_queue),
      woken: Cell::new(false),
    });
  ...
  fn atomic_load_head_and_len_all(&self) -> (*const Task<Fut>, usize) {
    let task = self.head_all.get();
    let len = if task.is_null() {
      0
    } else {
      unsafe {
        (*task).spin_next_all(self.pending_next_all());
        *(*task).len_all.get()
      }
    };
  ...
  /// the `Rc<Task>` or transfers ownership to the ready to run queue.
  /// The task this method is called on must have been unlinked before.
  fn release_task(&mut self, task: Rc<Task<Fut>>) {
    // `release_task` must only be called on unlinked tasks
    debug_assert_eq!(task.next_all.get(), self.pending_next_all());
    unsafe {
    ...
    let prev = task.queued.replace(true);
  ...
  fn link(&self, task: Rc<Task<Fut>>) -> *const Task<Fut> {
    ...
    unsafe {
      ...
      let new_len = if next.is_null() {
        ...
        (*next).spin_next_all(self.pending_next_all());
        *(*next).len_all.get() + 1
      };
  ...
  /// valid pointer.
  unsafe fn unlink(&mut self, task: *const Task<Fut>) -> Rc<Task<Fut>> {
    unsafe {
      ...
      let task = Rc::from_raw(task);
      let next = task.next_all.get();
      let prev = *task.prev_all.get();
      task.next_all.set(self.pending_next_all());
  ...
  fn pending_next_all(&self) -> *mut Task<Fut> {
    ...
    Rc::as_ptr(&self.ready_to_run_queue.stub) as *mut _
  }
}
    ...
    loop {
      ...
      let future = match unsafe { &mut *(*task).future.get() } {
        ...
        None => {
          ...
          let task = unsafe { Rc::from_raw(task) };
      ...
      let prev = task.queued.replace(false);
      assert!(prev);
      ...
      // our `Rc<Task<Fut>>` and can schedule the future for polling by
      ...
      match res {
        Poll::Pending => {
          ...
          yielded += task.woken.get() as usize;
...
impl<Fut> Drop for FuturesUnordered<Fut> {
  fn drop(&mut self) {
    ...
    impl<Fut> Drop for LeakQueueOnDrop<'_, Fut> {
      fn drop(&mut self) {
        mem::forget(Rc::clone(&self.0.ready_to_run_queue));
      }
    }
